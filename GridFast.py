...
class Grid:
    ...
    def create_grid(self):
        ...
        # Assign skepticism levels in alternating diagonals of S1 and S2
        for i in range(self.grid_size):
            for j in range(self.grid_size):
                if random.randint(1, 100) < 70:
                    if (i + j) % 2 == 0:
                        self.grid[i, j].type = 'S2'
                        self.grid[i, j].rumor_prob = Globals.transmission_probability.get('S2', None)
                        self.grid[i, j].original_params["type"] = 'S2'
                        self.grid[i, j].original_params["rumor_prob"] = Globals.transmission_probability.get('S2', None)
                    else:
                        self.grid[i, j].type = 'S1'
                        self.grid[i, j].rumor_prob = Globals.transmission_probability.get('S1', None)
                        self.grid[i, j].original_params["type"] = 'S1'
                        self.grid[i, j].original_params["rumor_prob"] = Globals.transmission_probability.get('S1', None)
