        ...
        for i in range(self.grid_size):
            for j in range(self.grid_size):
                if random.randint(1, 100) < 70:
                    if (i + j) % 2 == 0:
                        self.grid[i, j].type = 'S3'
                        self.grid[i, j].rumor_prob = Globals.transmission_probability.get('S3', None)
                        self.grid[i, j].original_params["type"] = 'S3'
                        self.grid[i, j].original_params["rumor_prob"] = Globals.transmission_probability.get('S3', None)
                    else:
                        self.grid[i, j].type = 'S4'
                        self.grid[i, j].rumor_prob = Globals.transmission_probability.get('S4', None)
                        self.grid[i, j].original_params["type"] = 'S4'
                        self.grid[i, j].original_params["rumor_prob"] = Globals.transmission_probability.get('S4', None)
